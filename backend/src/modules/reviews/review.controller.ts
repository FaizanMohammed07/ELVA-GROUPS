...
export const ReviewController = {
  async getProductReviews(req: Request, res: Response): Promise<void> {
    ...
    const reviews = await ReviewModel.find({ productId: req.params.productId, isApproved: true })
  ...
  async getReviewSummary(req: Request, res: Response): Promise<void> {
    const reviews = await ReviewModel.find({ productId: req.params.productId, isApproved: true }, 'rating');
  ...
  async createReview(req: Request, res: Response): Promise<void> {
    ...
    const review = await ReviewModel.create({ ...req.body, userId: req.user!.id });
  ...
  async deleteReview(req: Request, res: Response): Promise<void> {
    await ReviewModel.findOneAndDelete({ _id: req.params.id, userId: req.user!.id });
    sendSuccess(res, null, 'Review deleted');
  },

  async markHelpful(req: Request, res: Response): Promise<void> {
    await ReviewModel.findByIdAndUpdate(req.params.id, { $inc: { helpfulVotes: 1 } });
